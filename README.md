...
<details>
  ...
  <ol>
    ...
    <li>
      <a href="#project-code">Project sample code</a>
    </li>
  </ol>
</details>
...
# About The Project
 ...
 <p align="center"><img src="images/Figure5.png" width="700" alt="Logo"></p>
...
# Instructions
There are different codes to obtain the results of this project. 
- To train and test all algorithms (except MOM and KRL) on the BeatAML2 dataset run [executeBeatAML2.r](https://github.com/KatynaSada/InterpretableAIReview/blob/main/executeBeatAML2.r).
...
<!-- PROJECT CODE -->
# Project sample code
The code for training and testing all algorithms (except MOM and KRL) with mutations, expression and both on the BeatAML2 dataset is shown in detail below.
## Loading Libraries and Packages

First we load all the libraries and external functions required for the analysis.
...
We then loaded all the data for analysis.
The data selected were the BeatAML2 cohort for which we had mutational data, drug sensitivity data, and gene expression data.
...
Now it is time to preprocess the data to obtain two different matrices, one matrix containing all patients and their available mutations and a second matrix containing all patients and their sensitivity to the different drugs. To do this, we will impute the missing values in the drug matrix.
...
We performed 5-fold cross-validation using the BeatAML2 dataset.
We trained all models with genetic variant data, splitting the cohort between the 4-fold training samples and the 1-fold test samples.
We tested each of the folds, and compared the predicted IC50\* for the 5-fold test for all methods and compared it to Oracle-the drug with the optimal IC50\*.
...
* Use the codes in the MOM and KRL folders to compute the results.
## Mutation Models
<details><summary>Click to expand</summary>
...
We trained the models on the BeatAML cohort and measured the training time for each model except KRL and MOM, which were measured independently.
Technical implementation refers to the computational load and software required by the method. Although it could be considered less important, some of the algorithms require hours of computational time for BeatAML of the subset of AML samples in GDSC-which are considered small/medium size.
...
### Plotting BeatAML Test Results and comparing means difference
...
<details><summary>Click to expand</summary>
Some of the methods accept only binary variables as input.
Although genomic variants can be transformed into binary variables, gene expression, methylation or chromatin opening are intrinsically continuous variables.
...
For methods that accept continuous variables, we evaluated the performance of the predictions (5-fold cross-validation) on the BeatAML dataset using both data sources.
We declared statistical significance using a 2-tailed Wilcoxon test comparing IC50 using genetic variants or gene expression as input.
...
### Train Models with expression data for CV
```{r}
library(robustbase)
set.seed(2022)
folds <- 5
